...
interface LuxuryCardProps extends Omit<HTMLMotionProps<'div'>, 'children'> {
  ...
  shimmer?: boolean;
  tilt?: boolean;
  /** NOTE: We accept asChild to avoid React warnings if a parent passes it,
   * but we DO NOT forward it to the DOM element. */
  asChild?: boolean;
  className?: string;
}
...
export function LuxuryCard({
  ...
  tilt = false,
  className,
  asChild, // <- destructure so it is NOT forwarded to the DOM
  ...props
}: LuxuryCardProps) {
    ...
    >
      ...
      {variant === 'gradient' && (
        <>
          ...
          <div
            className="absolute bottom-6 left-6 w-1 h-1 bg-brand-turquoise/40 rounded-full animate-float"
            style={{ animationDelay: '1s' }}
          />
          <div
            className="absolute top-1/2 left-4 w-1.5 h-1.5 bg-brand-magenta/20 rounded-full animate-float"
...
export function LuxuryCardDescription({ children, className }: LuxuryCardDescriptionProps) {
  return <p className={cn('text-brand-muted leading-relaxed', className)}>{children}</p>;
}
